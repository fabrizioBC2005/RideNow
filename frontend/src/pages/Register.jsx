...
import { 
  LuUser, LuMail, LuPhone, LuKeyRound, 
  LuEye, LuEyeOff, LuArrowRight, LuMapPin 
} from 'react-icons/lu'
...
const registerSchema = z.object({
  ...
  phone: z.string()
    ...
    .min(11, "Faltan números")
    .regex(/^9/, "Debe iniciar con 9"),
  address: z.string().optional(),
  password: z.string().min(1, "Crea una clave").min(8, "Mínimo 8 caracteres"),
...
export default function RegisterPage() {
  ...
  const onSubmit = (data) => console.log("Registrando...", data)
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="group">
              <div className="flex items-center gap-3 bg-white/[0.02] border border-white/5 rounded-xl px-4 py-3.5 focus-within:border-yellow/40 transition-all">
                ...
                <input {...register("address")} placeholder="Dirección (Opcional)" className="bg-transparent text-white text-xs outline-none flex-1" />
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full bg-yellow text-night font-black py-4 rounded-xl mt-4 text-[11px] uppercase tracking-[0.2em] flex justify-center items-center gap-2 hover:brightness-110 active:scale-[0.98] transition-all shadow-xl shadow-yellow/5"
            >
