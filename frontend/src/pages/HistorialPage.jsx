...
import { LuCar, LuMapPin, LuNavigation, LuClock, LuCalendar, LuCircleCheck, LuCircleX, LuCreditCard } from 'react-icons/lu'
    ...
    <>
      ...
      <main className="bg-night min-h-screen pt-8 pb-16 px-6">
        <div className="w-full max-w-[680px] mx-auto">
          <div className="mb-8">
            <h1 className="text-2xl font-black text-white tracking-tight">Historial de viajes</h1>
            <p className="text-gray-500 text-sm mt-1">Todos tus viajes registrados</p>
          </div>
          <div className="grid grid-cols-3 gap-3 mb-8">
            <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 text-center">
              <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">Total viajes</p>
            ...
            <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 text-center">
              <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-1">Total gastado</p>
          ...
          {!cargando && viajes.length === 0 && (
            <div className="text-center py-20">
              <LuCar className="text-gray-700 mx-auto mb-4" size={48} />
              <p className="text-gray-500 font-bold">Aun no tienes viajes registrados</p>
          ...
          {!cargando && viajes.length > 0 && (
            <div className="space-y-3">
              {viajes.map((v, i) => {
                ...
                return (
                  <div key={v.id || i} className={"bg-white/[0.03] border rounded-2xl p-5 " + estado.border}>
                    <div className="flex items-start justify-between gap-3 mb-4">
                      <span className={"flex items-center gap-1.5 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg " + estado.bg + " " + estado.text}>
                        {v.estado}
                      </span>
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-start gap-2">
                        <LuNavigation className="text-yellow shrink-0 mt-0.5" size={13} />
                        <p className="text-white text-xs font-medium leading-snug">{v.origen}</p>
                      </div>
                      <div className="flex items-start gap-2">
                        <LuMapPin className="text-red-400 shrink-0 mt-0.5" size={13} />
                        <p className="text-white text-xs font-medium leading-snug">{v.destino}</p>
                      </div>
                    </div>
                    <div className="flex items-center justify-between pt-3 border-t border-white/5">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 rounded-full bg-yellow/10 border border-yellow/20 flex items-center justify-center">
                          <span className="text-yellow font-black text-[10px]">{CONDUCTORES[conductorIdx][0]}</span>
                        </div>
                        <div>
